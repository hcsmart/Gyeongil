...
const DEFAULTS = {
  '설비':[
    ['오버로드 프로텍트 (정상범위)','압력계 정상범위 · 경고등 소등'],
    ['발란스 실린더 (정상범위)','설정 압력 유지 · 누유 없음'],
    ['클러치 / 브레이크 (정상범위)','에어압 정상 · 제동거리 정상'],
    ['모터 (이상음)','이상음 · 이상진동 · 발열 없음'],
    ['클러치 (작동상태)','결합/해제 지연 없음'],
    ['안전장치 (센서, 양수조작버튼)','광전센서 · 양수조작 정상 동작'],
    ['오일러 급유 (적정량, 흐름상태)','유면 MIN 이상 · 적하 확인'],
    ['피치세팅 10회 체크','10회 연속 피치 편차 없음'],
    ['안전보호구 착용','크레인/사다리 사용 시 안전모 착용']
  ],
  '금형':[
    ['금형 외관 · 균열 확인','크랙 · 파손 · 변형 없음'],
    ['펀치 / 다이 상태','치핑 · 마모 · 버 발생 없음'],
    ['스프링 · 리프터 파손','절손 · 처짐 없음'],
    ['가이드 포스트 급유','급유 상태 양호 · 소착 없음'],
    ['스크랩 배출 상태','스크랩 막힘 없음'],
    ['미스피드 / 센서 작동','센서 감지 및 정지 동작 정상'],
...
async function init(target, pageId){
  ...
  if(KI.can(pageId,'save')) acts.push(['＋ 항목 등록','primary',()=>open(null)],
                                      ...
                                      ['📊 엑셀 업로드','',()=>uploader.open()]);
  acts.push(['새로고침','',()=>load()]);
  const ui=KI.page(cur,acts);

  /* ---------- 엑셀 템플릿 업로드 ---------- */
  const upDef={edit:{table:T, pk:'check_id', auto:true, fixed:{target:target}, fields:[
    ['check_id','번호','num'],
    ['check_type','점검구분','sel',['일상','정기'],'req'],
    ['item_name','점검항목','text',null,'req'],
    ['criteria','판정기준','area'],
    ['cycle','주기','sel',CYCLES.filter(Boolean)],
    ['sort_order','표시순서','num'],
    ['is_active','사용','bool',{def:true}]
  ]}};
  const uploader=KI.makeUpload(upDef, target+'_점검기준', ()=>ROWS,
    async n=>{ await load(); KI.msg('✓ 엑셀 업로드 '+n+'건이 저장되었습니다.'); });
  ...
  nt.innerHTML='여기서 등록한 항목이 <b>일상점검 · 정기점검</b> 화면에 그대로 표시됩니다. '+
    '엑셀로 일괄 관리하려면 [📊 엑셀 업로드]에서 양식을 내려받아 작성 후 업로드하세요.';
  ...
  async function loadDefaults(){
    ...
    try{
      await KI.ins(T, add.map((x,i)=>({target:target,check_type:'일상',item_name:x[0],criteria:x[1],
        sort_order:n+i+1,is_active:true})));
  ...
  function draw(){
    ...
    if(!list.length){
      ...
      d.colSpan=5; d.style.cssText='color:#8894a0;height:56px'; t.appendChild(d); tbody.appendChild(t);
    }
    list.forEach((r,i)=>{
      const t=el('tr');
  ...
  async function load(){
    try{
      ...
      tbody.innerHTML='<tr><td colspan="5" class="center" style="color:#b0442f;height:50px">❌ '+esc(e.message)+'</td></tr>';
    }
  }
  load();
}
