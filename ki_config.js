...
const HIDE = {
  ...
  /* [LOT 추적(통합조회)]와 겹치는 화면만 메뉴에서 숨긴다.
     ...
     lot-scan(QR 수동·수정) : 협력사는 qr_home.html → QR 리딩만 쓰고,
       본사 등록·정정은 [물류등록] 화면이 정식 경로다. 또한 이 화면은 반송·운송중
       개념이 없는 옛 로직이라 여기서 등록하면 수량 집계가 어긋날 수 있어 숨긴다.
       (파일·권한은 그대로 두므로 필요하면 이 배열에서 지우면 다시 나온다) */
  item: ['lot-move','lot-token','lot-scan']
};
