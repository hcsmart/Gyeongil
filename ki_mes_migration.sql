...
/* 연마 · 교체 · 일별 타발수 관련 뷰 (화면은 있었으나 뷰가 없어 404 였던 것들) */
...
create view public.ki_v_mold_tool with (security_invoker=true) as
  select t.mold_code, m.mold_name, m.customer_name, m.machine_no,
         t.kind, t.part_name, t.use_common,
         t.limit_shot, t.cycle_days, t.warn_pct,
         t.last_date, t.last_shot, m.shot_count,
         t.is_active, t.remark, t.updated_at
...
create view public.ki_v_tool_alert with (security_invoker=true) as
  select a.alert_id, a.occurred_at, a.mold_code, m.mold_name, m.machine_no,
         a.kind, a.part_name, a.level, a.reason,
         a.shot_count, a.limit_shot, a.used_shot, a.days_left,
         a.message, a.status, a.notified, a.notified_at,
         a.action, a.closed_at, a.closed_by
    from public.ki_tool_alert a
    left join public.ki_mold m on m.mold_code = a.mold_code;

/* 일별 타발수 : cum_shot = 기초 타발수 + 그 날짜까지의 일별 합계 */
drop view if exists public.ki_v_shot_daily cascade;
create view public.ki_v_shot_daily with (security_invoker=true) as
  select d.mold_code, d.work_date,
         m.mold_name, m.customer_name, m.grade, m.shot_limit,
         coalesce(d.machine_no, m.machine_no) as machine_no,
         d.shot_qty, d.counter_end,
         coalesce(m.shot_base,0) + coalesce((
           select sum(x.shot_qty) from public.ki_shot_daily x
            where x.mold_code = d.mold_code and x.work_date <= d.work_date),0) as cum_shot,
         d.worker, d.remark, d.updated_at
    from public.ki_shot_daily d
    left join public.ki_mold m on m.mold_code = d.mold_code;

revoke all on public.ki_v_tool_rule, public.ki_v_mold_tool,
              public.ki_v_tool_alert, public.ki_v_shot_daily from anon;
grant select on public.ki_v_tool_rule, public.ki_v_mold_tool,
                public.ki_v_tool_alert, public.ki_v_shot_daily to authenticated;

insert into public.ki_table_menu(table_name, menus) values
 ('ki_tool_rule',  array['tool-rule']),
 ('ki_mold_tool',  array['tool-mold','tool-due']),
 ('ki_tool_alert', array['tool-due','tool-alarm']),
 ('ki_shot_daily', array['shot-daily','shot-ledger'])
on conflict (table_name) do update set menus = excluded.menus;
